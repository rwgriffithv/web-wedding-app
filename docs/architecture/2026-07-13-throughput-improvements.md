...
- **RSVP deadline timezone:** The `datetime-local` input produces a local time string (e.g., `"2025-08-15T17:00"`). When parsed with `new Date()`, JavaScript interprets it as local time in the server's timezone. Since the server runs in Pacific time and the admin is also in Pacific time, this works correctly. If the server timezone ever changes, this logic must be updated.
