...
export function getPlusOneCount(): { attending: number; not_attending: number } {
  const db = getDb();
  ...
  const not_attending = db.prepare(`SELECT COUNT(*) as count FROM rsvp_responses WHERE attending = 0 AND ${wherePlusOne}`).get() as { count: number };
  return { attending: attending.count, not_attending: not_attending.count };
}
